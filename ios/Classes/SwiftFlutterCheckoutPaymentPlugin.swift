public class SwiftFlutterCheckoutPaymentPlugin: NSObject, FlutterPlugin {
    ...
    private var METHOD_GENERATE_CVV_TOKEN : String = "generateCVVToken"
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        } else if call.method == METHOD_GENERATE_CVV_TOKEN {
            // Get the args from Flutter.
            let args = call.arguments as? [String: Any]
            let cvv : String = args!["cvv"] as! String

            checkoutAPIClient.createSecurityCodeToken(securityCode: cvv) { createTokenResult in
                do {
                    switch createTokenResult {
                    case let .success(tokenDetails):
                        let tokenResponse = try self.tokenCVVDetailsToTokenisationResponseString(tokenDetails: tokenDetails)
                        result(tokenResponse)
                    case .failure(let ex):
                        result(FlutterError(code: self.GENERATE_TOKEN_ERROR, message: ex.localizedDescription, details: nil))
                    }
    ...
    fileprivate func tokenCVVDetailsToTokenisationResponseString(tokenDetails: TokenDetails) throws -> String {

        let response = SecurityCodeResponse(type: tokenDetails.type, token: tokenDetails.token, expiresOn: tokenDetails.expiresOn)
